...
Added a texture presets feature, this allows for you to select a texture from a menu, so that when settings the texture for things such as items and pages in the player shops, you don't have to know the exact path of the texture.
...
-   Made the following changes to a few of the `savedPlayer`{lang=ts} class:
