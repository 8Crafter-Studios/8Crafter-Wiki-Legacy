...
var createObserversSetup = (t2, n2, o2, s2) => {
  ...
  const { vt: p2, ht: v2, U: h2, gt: g3, bt: b2, L: w2, wt: y2, yt: S2, St: m2, Ot: O2 } = t2;
  ...
  const x3 = getStaticPluginModuleInstance(xt);
  const [H2] = createCache({
    ...
    const s3 = x3 && x3.V(t2, n2, $2, C2, o2).W;
    const e3 = y2 && w2;
    const c3 = !e3 && hasAttrClass(v2, B, N);
    const r3 = !w2 && S2(W);
    const l3 = r3 && getElementScroll(g3);
    const i3 = l3 && O2();
    const a3 = m2(J, c3);
    const u3 = r3 && s3 && s3()[0];
    const f3 = M(h2);
  ...
  return [() => {
    ...
    const [g4, y3] = t3("update.debounce");
    const S3 = v3 || d2;
    const m3 = n3 || o3;
...
var createScrollbarsSetupElements = (t2, n2, o2, s2) => {
  ...
  const { slot: a2 } = i2;
  const { vt: u2, ht: f2, U: _2, Mt: d2, gt: v2, wt: h2, L: g3 } = n2;
  const { scrollbars: b2 } = d2 ? {} : t2;
  const { slot: w2 } = b2 || {};
  const y2 = [];
  const S2 = [];
  const m2 = [];
  const O2 = dynamicInitializationElement([u2, f2, _2], () => g3 && h2 ? u2 : f2, a2, w2);
...
var createScrollbarsSetupEvents = (t2, n2, o2, s2) => (r2, l2, i2) => {
  ...
  const createInteractiveScrollEvents = () => {
    ...
    const f3 = [];
    return addEventListener(g3, "pointerdown", (o3) => {
      ...
      if (x3) {
        ...
        const k2 = [addEventListener(p2, n3, releasePointerCapture), addEventListener(p2, "selectstart", (t4) => preventDefault(t4), {
          H: false
        }), addEventListener(g3, n3, releasePointerCapture), M2 && addEventListener(g3, "pointermove", (t4) => $3(D2 + (t4[s3] - C3))), M2 && (() => {
...
var createScrollbarsSetup = (t2, n2, o2, s2, e2, c2) => {
  ...
  const [h2, g3] = selfClearTimeout(100);
  ...
  const T2 = [S2, g3, w2, v2, () => f2(), addEventListener($2, "pointerover", onHostMouseEnter, {
    A: true
  ...
  return [() => bind(runEachAndClear, push(T2, O2())), ({ It: t3, Dt: n3, Zt: e3, tn: c3 }) => {
    ...
    const [P3, N2] = t3("scrollbars.autoHideSuspend");
...
var createStructureSetupElements = (t2) => {
  ...
  const g3 = v2.ownerDocument;
  const b2 = g3.documentElement;
  const getDocumentWindow = () => g3.defaultView || n;
...
var createPaddingUpdateSegment = ({ ht: t2, ln: n2, U: o2, L: s2 }, e2) => {
  ...
  return ({ It: t3, Zt: l2, fn: i2, Dt: a2 }) => {
    ...
    const { F: O2 } = i2;
    const [C2, x3] = t3("paddingAbsolute");
    const H2 = a2 || p2;
    if (d2 || f2 || H2) {
      [u2, f2] = c2(a2);
    }
...
var createOverflowUpdateSegment = (t2, s2) => {
  ...
  const P3 = getStaticPluginModuleInstance(xt);
  ...
  return ({ It: n2, Zt: o2, fn: i2, Dt: a3 }, { dn: u3 }) => {
    ...
    const S3 = P3 && P3.V(t2, s2, i2, e2, n2);
    const { Y: x3, W: H2, J: E2 } = S3 || {};
    ...
    if (_t2) {
      ...
      if (E2 && x3) {
        setStyles(l2, E2(dt2, i2, x3(dt2, ct2, lt2)));
      }
    }
...
var createSetups = (t2, n2, o2, s2, e2) => {
  ...
  const [v2, h2, , g3] = createScrollbarsSetup(t2, n2, p2, a2, u2, e2);
  ...
  const update = (t3, e3) => {
    ...
    const w2 = updateHintsAreTruthy(g4);
...
var OverlayScrollbars = (t2, n2, o2) => {
  ...
  if (n2 && !r2) {
    ...
    const [h2, g3, b2, w2, y2] = createSetups(t2, a2, () => r3, ({ vn: t3, Dt: n3 }, { Zt: o3, tn: s3 }) => {
