...
if(!!sourceURL){
  ...
  [
    ...
    if (window.jQuery) {
      $("article").on("mouseenter mouseleave", ".animated-container, .mcui", function (e) {
        $(this)
          .find(".animated")
        ...
        $("article").on(
          {
            "mouseenter.minetip": function (e) {
              ...
              if (
                title === undefined ||
                (title !== "" &&
                  title.replace(/\\u0026([0-9a-jl-qs-vyzr]|#[0-9a-fA-F]{6}|\\$[0-9a-fA-F]{3})/g, "") === "")
              ) {
    ...
    console.log("minetip loaded")
        ...
        setTimeout(function() { waitForjQueryLoad() }, 50);
    }
}
waitForjQueryLoad();`,
  ],
